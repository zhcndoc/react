...
`renderToPipeableStream` 将一个 React 树渲染到可管道传输的 [Node.js Stream。](https://nodejs.org/api/stream.html)
...
此 API 仅适用于 Node.js。支持 [Web Streams,](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API) 的环境，例如 Deno 和现代边缘运行时，应改用 [`renderToReadableStream`](/reference/react-dom/server/renderToReadableStream)。
...
调用 `renderToPipeableStream`，将你的 React 树渲染为 HTML，并输出到 [Node.js Stream。](https://nodejs.org/api/stream.html#writable-streams)
...
在客户端，调用 [`hydrateRoot`](/reference/react-dom/client/hydrateRoot)，使服务端生成的 HTML 变得可交互。
...
* `pipe` 将 HTML 输出到提供的 [可写 Node.js Stream。](https://nodejs.org/api/stream.html#writable-streams) 如果你想启用流式传输，就在 `onShellReady` 中调用 `pipe`；如果是爬虫和静态生成，则在 `onAllReady` 中调用。
* `abort` 允许你 [中止服务端渲染](#aborting-server-rendering) 并在客户端渲染剩余部分。

---

## 用法 {/*usage*/}

### 将 React 树渲染为 Node.js Stream 的 HTML {/*rendering-a-react-tree-as-html-to-a-nodejs-stream*/}
...
除了 <CodeStep step={1}>根组件</CodeStep> 之外，你还需要提供一组 <CodeStep step={2}>bootstrap `<script>` 路径</CodeStep>。你的根组件应该返回**整个文档，包括根部的 `<html>` 标签。**
...
在客户端，你的 bootstrap 脚本应该通过调用 [`hydrateRoot`:](/reference/react-dom/client/hydrateRoot#hydrating-an-entire-document) [将整个 `document` 水合：]
...
这会把事件监听器附加到服务端生成的 HTML 上，并使其可交互。
...
客户端和服务端都使用相同的 `assetMap` prop 渲染 `App`，因此不会出现水合错误。

</DeepDive>

---

### 随着内容加载而流式输出更多内容 {/*streaming-more-content-as-it-loads*/}
...
想象一下，加载 `<Posts />` 的数据需要一些时间。理想情况下，你会希望在不等待帖子加载的情况下，先向用户展示个人主页的其余内容。为此，请将 `Posts` [包裹在 `<Suspense>` 边界中：](/reference/react/Suspense#displaying-a-fallback-while-content-is-loading)
...
你还可以进一步 [嵌套 `<Suspense>` 边界，](/reference/react/Suspense#revealing-nested-content-as-it-loads) 以创建更细粒度的加载顺序：
...
[进一步了解流式 HTML 的工作方式。](https://github.com/reactwg/react-18/discussions/37)
...
### 指定哪些内容进入 shell {/*specifying-what-goes-into-the-shell*/}
...
如果你把整个应用在根部包进一个 `<Suspense>` 边界，那么 shell 就只会包含那个转圈加载指示器。然而，这并不是一个令人愉快的用户体验，因为屏幕上看到一个大转圈可能比再多等一会儿直接看到真实布局更慢、更烦人。这就是为什么通常你会希望放置 `<Suspense>` 边界，使 shell 感觉*尽可能精简但完整*——就像整个页面布局的骨架一样。
...
### 记录服务器崩溃日志 {/*logging-crashes-on-the-server*/}
...
### 从 shell 内部错误中恢复 {/*recovering-from-errors-inside-the-shell*/}
...
### 从 shell 外部错误中恢复 {/*recovering-from-errors-outside-the-shell*/}

在这个示例中，`<Posts />` 组件被包裹在 `<Suspense>` 中，因此它*不属于* shell：
...
1. 它会把最近的 `<Suspense>` 边界（`PostsGlimmer`）的加载占位输出到 HTML 中。
2. 它会“放弃”继续尝试在服务端渲染 `Posts` 内容。
3. 当 JavaScript 代码在客户端加载后，React 会在客户端*重试*渲染 `Posts`。

如果在客户端重试渲染 `Posts` 时也失败了，React 会在客户端抛出该错误。和所有在渲染期间抛出的错误一样，[最近的父错误边界](/reference/react/Component#static-getderivedstatefromerror) 决定如何向用户展示错误。实际上，这意味着用户会看到一个加载指示器，直到确定该错误不可恢复为止。

如果在客户端重试渲染 `Posts` 成功了，服务端的加载占位会被客户端渲染结果替换。用户不会知道曾经有过服务端错误。不过，服务端的 `onError` 回调和客户端的 [`onRecoverableError`](/reference/react-dom/client/hydrateRoot#hydrateroot) 回调都会触发，这样你就能收到错误通知。
...
通过将你的应用[划分](#specifying-what-goes-into-the-shell)为 shell（位于所有 `<Suspense>` 边界之上）和其余内容，你已经解决了这个问题的一部分。如果 shell 出错，你会得到 `onShellError` 回调，它允许你设置错误状态码。否则，你就知道应用可能会在客户端恢复，因此可以发送“OK”。
...
如果某个位于 shell *之外* 的组件（即在 `<Suspense>` 边界内）抛出错误，React 不会停止渲染。这意味着 `onError` 回调会触发，但你仍然会得到 `onShellReady`，而不是 `onShellError`。这是因为 React 会尝试在客户端从该错误中恢复，[如上所述。](#recovering-from-errors-outside-the-shell)
...
你可以[创建自己的 `Error` 子类](https://javascript.info/custom-errors)，并使用 [`instanceof`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/instanceof) 运算符检查抛出了哪种错误。例如，你可以定义一个自定义的 `NotFoundError` 并在组件中抛出它。然后你的 `onError`、`onShellReady` 和 `onShellError` 回调就可以根据错误类型做不同的处理：
...
### 等待所有内容加载完毕，供爬虫和静态生成使用 {/*waiting-for-all-content-to-load-for-crawlers-and-static-generation*/}
...
普通访问者会收到逐步加载的内容流。爬虫会在所有数据加载完成后收到最终 HTML 输出。不过，这也意味着爬虫必须等待*所有*数据，其中一些可能加载很慢，或者出错。根据你的应用情况，你也可以选择向爬虫发送 shell。
...
React 会把剩余的加载占位作为 HTML 刷出，并尝试在客户端渲染其余部分。
