...
类组件的 [上下文](/learn/passing-data-deeply-with-context) 可通过 `this.context` 访问。只有在你使用 [`static contextType`](#static-contexttype) 指定了*要接收哪个* context 时，它才可用。
...
如果你定义了 `componentDidCatch`，当某个子组件（包括更深层级的子组件）在渲染过程中抛出错误时，React 会调用它。这让你可以在生产环境中将该错误记录到错误报告服务。
...
此 API 已从 `componentWillMount` 重命名为 [`UNSAFE_componentWillMount`.](#unsafe_componentwillmount) 旧名称已被弃用。在未来的 React 主要版本中，只有新名称可用。
...
此 API 已从 `componentWillReceiveProps` 重命名为 [`UNSAFE_componentWillReceiveProps`.](#unsafe_componentwillreceiveprops) 旧名称已被弃用。在未来的 React 主要版本中，只有新名称可用。
...
This API has been renamed from `componentWillUpdate` to [`UNSAFE_componentWillUpdate`.](#unsafe_componentWillUpdate) The old name is deprecated. In future major versions of React, only the new name will be available.
...
`setState` will enqueue changes to the component state. It tells React that this component and its child components need to re-render with the new state. This is the main way you respond to interactions and update the user interface.

<Pitfall>

Calling `setState` **does not** change the current state in the code that is already running:
...
It only affects what `this.state` returns starting from the *next* render.
...
You don’t have to do this, but it can be useful if you want to update state multiple times in the same event.

#### Parameters {/*setstate-parameters*/}

* `nextState`: Can be an object or a function.
  * If you pass an object as `nextState`, it will be shallowly merged into `this.state`.
  * If you pass a function as `nextState`, it will be treated as an _updater function_. It must be pure, should receive the pending state and props as arguments, and should return an object that will be shallowly merged into `this.state`. React will put your updater function in a queue and re-render the component. During the next render, React will calculate the next state by applying all of the queued updater functions to the previous state.

* **optional** `callback`: If specified, React will call the `callback` you provided after the update is committed.
...
- Think of `setState` as a *request* rather than an immediate command to update the component. When multiple components update their state in response to an event, React may batch their updates and re-render them together at the end of the event. In the rare case that you need to force a state update to be applied synchronously, you can wrap it in [`flushSync`,](/reference/react-dom/flushSync) but this may hurt performance.

- `setState` does not update `this.state` immediately. Therefore, reading `this.state` right after calling `setState` is a pitfall. Instead, use [`componentDidUpdate`](#componentdidupdate) or the `callback` parameter to `setState`, both of which are guaranteed to fire after the update has been applied. If you need to set state based on the previous state, you can pass a function to `nextState` as described above.

<Note>

Calling `setState` in class components is similar to calling the [`set` function](/reference/react/useState#setstate) in function components.
...
class Button extends Component {
  ...
  render() {
    return <button className={this.props.color}>点击我</button>;
  }
}
```
...
如果你定义了 `static getDerivedStateFromError`，当某个子组件（包括更深层级的子组件）在渲染过程中抛出错误时，React 会调用它。这让你可以显示错误信息，而不是清空 UI。
...
- 此方法会在*每次*渲染时触发，不论原因是什么。这与 [`UNSAFE_componentWillReceiveProps`](#unsafe_componentwillreceiveprops) 不同，后者只会在父组件导致重新渲染时触发，而不会因为本地 `setState` 而触发。
